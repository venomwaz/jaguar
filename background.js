    ...
    ({ tabId }) => {
        ...
        chrome.tabs.get(tabId, (tab) => {
            ...
            if (
                tab.url &&
                tab.url.includes(
                    "mcdonalds-poland.atlassian.net"
                )
            ) {

                lastActiveJiraTab = tabId;
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
    if (message.type === "checkSession") {
        ...
        if (message.type === "GetLastJiraTab") {

    sendResponse({
        tabId: lastActiveJiraTab
    });
...
.catch(error => {
    ...
    //             fetch(`https://mcdonalds-poland.atlassian.net/rest/api/2/issue`, {
    //             method: "POST",
    //             headers: {
    //                 "Authorization": `Basic ${auth}`,
    //                 "Accept": "application/json",
